...
# taken from https://github.com/MarkBridge11/ZeroFake-Mod 
from sentence_transformers import SentenceTransformer, util

sentence_transformer = SentenceTransformer("all-MiniLM-L6-v2")

word_path = os.path.join(dir_path, "words/most-common-nouns-english.csv")
import pandas as pd
...
for impath in images_list_str:
    ...
    print(prompt)
    original_embedding = sentence_transformer.encode(prompt)

    candidate_prompts = get_perturbed_prompts(prompt)

    least_similar_adv_prompt = None
    min_sim = 1.0
    for adv_prompt in candidate_prompts:
        adv_embedding = sentence_transformer.encode(adv_prompt)
        sim = util.pytorch_cos_sim(adv_embedding, original_embedding)[0]
        if sim < min_sim:
            min_sim = sim 
            least_similar_adv_prompt = adv_prompt

    # prompt = replace_first_noun(prompt, 'big tree')
    prompt = least_similar_adv_prompt
    print(prompt)
